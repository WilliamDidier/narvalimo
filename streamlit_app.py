...
def calculer_amortissement(emprunt, mensualite, taux_mensuel, duree):
   df = pd.DataFrame(columns=['Mois', 'Capital restant', 'Intérêts', 'Capital remboursé'])
...
def app():
   st.title("Bien")
   prix = int(st.number_input('Coût du bien', min_value=0, value=545000))
   superficie_totale = st.number_input('Superficie totale du bien en m²', min_value=20, value=180)
  
   st.title("Emprunt")
   taux = st.slider("Taux d'emprunt", min_value=0.0, max_value=7.0, step=0.01, value=3.8)
   duree = st.slider("Durée du prêt en mois", max_value=25*12, min_value=5*12, value=20*12)
   ...
   mensualite_totale = emprunt_total * (taux_mensuel * (1 + taux_mensuel) ** duree) / ((1 + taux_mensuel) ** duree - 1)


   superficie_chambres = df['Superficie chambre'].sum()
   poids_chambres = st.slider("Poids des chambres dans la répartition des parts (laisser par défaut pour une répartition au m²)", min_value = 0.0, max_value = 1.0, value = superficie_chambres/superficie_totale, step = .01)
   ...
   for idx, row in df.iterrows():       
       ...
       with cols[0]:
           st.write(row["Nom"])
       with cols[1]:
           part_detenue = row["Superficie chambre"] / superficie_chambres * poids_chambres + 1 / len(HABITANTS) * (1 - poids_chambres)
   ...
   # Onglet global
   with tab_global:
       df_amort_global = calculer_amortissement(emprunt_total, mensualite_totale, taux_mensuel, duree)
       fig_global = creer_graphique_amortissement(df_amort_global, "Évolution de l'amortissement global dans le temps")
       ...
       st.download_button(
           label=f"📥 Télécharger le tableau d'amortissement global",
           data=df_amort_global.to_csv(index=False),
           file_name=f"amortissement.csv",
           mime="text/csv"                   
       )
       with st.expander(f"👀 Afficher le tableau d'amortissement"):
   ...
   for tab, (idx, row) in zip(tabs_individuels, df.iterrows()):
       with tab:
           df_amort_indiv = calculer_amortissement(row['Emprunt'], row['Mensualite'], taux_mensuel, duree)
   ...
   if superficie_communs < 0:
       st.error(f"Erreur : La somme des superficies des chambres dépasse la superficie totale du bien : {superficie_chambres} > {superficie}")
       return


   fig = go.Figure(data=[go.Pie(labels=['Superficie des chambres', 'Superficie des parties communes'],
